    ...
    refreshdata() {
      getQdnameCount("开发中", "联调中").then((response) => {
        ...
        for (var i = 0; i < resdata.length; i++) {
          ...
          devCounts = devCounts + parseInt(resdata[i].nums);
        }
      ...
      getQdnameCount("一轮测试中", "验证测试中").then((response) => {
        ...
        for (var i = 0; i < resdata.length; i++) {
          ...
          testCounts = testCounts + parseInt(resdata[i].nums);
        }
        for (var i = 0; i < kfNames1.length; i++) {
          kfNames2.push(kfNames[i]);
          datas2.push(parseInt(datas[i]) + parseInt(datas1[i]));
          allCounts = allCounts + parseInt(datas[i]) + parseInt(datas1[i]);
        }
